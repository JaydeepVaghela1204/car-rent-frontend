...
export default class createCar extends Component {
    ...
    handleFile = (event) => {
        ...
        if (files !== null) {
            ...
            for (let i = 0; i < files[0].length; i++) {
                if (files[0][i].size > FILE_UPLOAD_LIMIT) {
                ...
                reader.onload = (event) => {
                    this.state.b64photo.push(event.target.result);
                }
            }
            // this.setState({
            //     b64photo: photo,
    ...
    inputValidation = () => {
        const { make, seats, bodytype, numberplate, colour, costperhour, fueltype, location, b64photo } = this.state;
        // const letter = /^[A-Z]{2}[ -][0-9]{1,2}(?: [A-Z])?(?: [A-Z]*)? [0-9]{4}$/;

        if (make === '' || seats === '' || bodytype === '' || numberplate === ''
            || colour === '' || costperhour === '' || fueltype === '' || location === '' || b64photo.length === 0) {
            this.setState({
